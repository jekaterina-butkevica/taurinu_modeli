...
sugaEGV="ERELIG"
suga="ERELIG"
suga_versija=paste0(suga,"_cv_v0b5")
home_range=500
...
grassland_folder = "GrasslandIndex" # Šeit ieraksti vēlamo mapes nosaukumu
...
klatbutnes_tikla=klatbutnes_sf %>% 
...
fonavietas3_tikla=fonavietas3_sf %>% 
  filter(!duplicated(id)) 
...
objekti2=objekti %>% 
  filter(!(objekti %in% c("grupa","suga","suga_versija","vpi_slieksnis")))
...
fons_kopa=rbind(trenin_dati@coords[trenin_dati@pa == 0, ])
...
fons_kopa=trenin_dati@coords[trenin_dati@pa == 0, ]
fonsX=data.frame(fons_kopa) %>% 
  mutate(x=X,
         y=Y) %>% 
  dplyr::select(x,y)
...
labakais_proc=SDMtune::plotROC(labakais_comb)
labakais_proc
...
#nullem_test=read_rds(paste0("./SpeciesModels/",
#                            grupa,"/",
#                            suga_versija,"/",
#                            "Model_NullsTest_",suga,".RDS"))
